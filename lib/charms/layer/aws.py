...
def tag_instance_security_group(instance_id, region, tags):
    ...
    log(
        "Tagging security group {} for instance {} in {} with: {}",
        group_name,
        instance_id,
        region,
        tags,
...
def tag_instance_subnet(application_name, instance_id, region, tags):
    ...
    """
    log("Tagging subnet for instance {} in {} with: {}", instance_id, region, tags)
    subnet_id = _aws(
        "ec2",
        "describe-instances",
        "--instance-ids",
        ...
        region,
        "--query",
        "Reservations[*]" ".Instances[*]" ".SubnetId[] | [0]",
    )
    if subnet_id is not None:
        _apply_tags(region, [subnet_id], tags)
        _add_app_entity(
            application_name, "subnet-tags", ((region, subnet_id), list(tags.keys()))
        )
    else:
...
def update_policies():
    ...
    for request in aws.all_requests:
        if (
            request.requested_object_storage_access
            ...
            _update_policy(policy_name, policy_arn)
        if (
            request.requested_object_storage_management
...
def cleanup(current_applications):
    ...
    if not departed_applications:
        return
    log("Cleaning up {} AWS entities", "unused" if current_applications else "all")
...
def _list_roles(model_uuid=None):
    ...
    return _aws(
        ...
        "list-roles",
        "--query",
        _build_query("Roles", "RoleName", model_uuid=model_uuid),
    )
...
def _list_instance_profiles(model_uuid=None):
    ...
    return _aws(
        ...
        "--query",
        _build_query("InstanceProfiles", "InstanceProfileName", model_uuid=model_uuid),
    )
...
def _list_policies(model_uuid=None):
    ...
    return _aws(
        ...
        "list-policies",
        "--query",
        _build_query("Policies", "PolicyName", "Arn", model_uuid=model_uuid),
    )
...
def _list_subnet_tags(region):
    results = _aws(
        ...
        region,
        "--query",
        "Subnets[*].[SubnetId,Tags]",
    )
    return {
        subnet_id: [tag["Key"] for tag in tags] for subnet_id, tags in results if tags
    }
...
def _add_app_entity(app_name, entity_type, entity_name):
    ...
    """
    managed_entities = _get_managed_entities()
    app_entities = managed_entities.setdefault(
        app_name,
        {
            "role": [],
            "instance-profile": [],
...
def _apply_tags(region, resources, tags):
    ...
    """
    tags = ["Key={},Value={}".format(key, value or "") for key, value in tags.items()]
    _aws(
        *(
            ["ec2", "create-tags"]
            + ["--region", region]
...
def _get_account_id():
    ...
    if not account_id:
        account_id = _aws("sts", "get-caller-identity", "--query", "Account")
...
def _policy_needs_update(policy_arn):
    ...
    policy_versions = _aws("iam", "list-policy-versions", "--policy-arn", policy_arn)
    cur_policy_ver = [
        v["VersionId"] for v in policy_versions["Versions"] if v["IsDefaultVersion"]
    ][0]
    cur_policy_doc = _aws(
        "iam",
        "get-policy-version",
...
def _ensure_role_attached(role_name, instance_id, region):
    ...
    def _associate_iam_instance_profile():
        try:
            ...
            log(
                "Attached IAM instance-profile {} to instance {} " "in region {}",
                role_name,
                instance_id,
                region,
            )
...
def _cleanup_role(role_name):
    ...
    try:
        ...
        log("Detached IAM role {} from instance-profile {}", role_name, role_name)
    except DoesNotExistAWSError:
        pass
    try:
        _aws("iam", "delete-role", "--role-name", role_name)
...
def _cleanup_policy(policy_arn):
    ...
    """
    try:
        versions = _aws("iam", "list-policy-versions", "--policy-arn", policy_arn)
        for version in versions["Versions"]:
            ...
            log("Deleted IAM policy {} version {}", policy_arn, version["VersionId"])
        _aws("iam", "delete-policy", "--policy-arn", policy_arn)
        policy_name = _get_policy_name(policy_arn)
        if _is_restricted_policy(policy_name):
            policy_file = _get_policy_file(policy_name)
    ...
    try:
        _aws(
            *(
                ...
                + ["--resources", subnet_id]
                + ["--tags"]
                + ["Key={}".format(tag) for tag in tags]
            )
        )
